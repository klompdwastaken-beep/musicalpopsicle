...
# Set the exact time you want the songs to be sent daily. 
# It is highly recommended to use UTC time so it doesn't break if your server changes timezones.
# 12:00 UTC is roughly morning in the Americas, and evening in Europe/Asia.
DELIVERY_TIME = datetime.time(hour=12, minute=0, tzinfo=datetime.timezone.utc)
...
class SubscriptionView(discord.ui.View):
    ...
    async def subscribe_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            # Send ONLY the welcome message. The first song will arrive at the scheduled time.
...
async def daily_music_delivery():
    ...
    print(f"[{datetime.datetime.now(datetime.timezone.utc)}] Scanning YouTube for today's song...")
...
async def before_delivery():
    ...
    print(f"Daily music task scheduled. Next delivery is at {DELIVERY_TIME} UTC.")
